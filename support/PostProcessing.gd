...
func update_img() -> void:
	...
	texture.create(FORMAT, RESOLUTION, RESOLUTION, RESOLUTION, false, img_arr)
	material.set_shader_parameter("noise", texture)
	
	time_end = Time.get_ticks_usec()
	print("Texture updated: %f ms" % ((time_end - time_start) / 1000))
...
func load_images_at(dir_path: String) -> void:
	...
	for i in range(resolution):
		var from: int = offsets[i]
		var to: int = offsets[i+1] if i < resolution - 1 else file.get_length() + 1
		file.seek(from)
	...
	if material.get_shader_parameter("points") != null:
		var shader_points = PackedVector3Array()
		shader_points.resize(CELLS*CELLS*CELLS)
		for z_cell in range(CELLS):
			for y_cell in range(CELLS):
				for x_cell in range(CELLS):
					var i: int = (z_cell * CELLS + y_cell) * CELLS + x_cell
					shader_points[i] = points[i] + Vector3(x_cell, y_cell, z_cell) / CELLS
		material.set_shader_parameter("points", shader_points)
	
	print("Cloud noise loaded in %f ms" % ((Time.get_ticks_usec() - time_start) / 1000))
